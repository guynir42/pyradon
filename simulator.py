...
from utils import empty, scalar, imsize, gaussian2D
...
class Simulator:
    ...
    def makeImage(self):
        ...
        if self.debug_bit > 1: print("makeImage()")
    ...
    def makeClean(self):
        self.image_clean = self.intensity*pyradon.utils.model(self.im_size, self.x1*self.im_size,
                                                              self.x2*self.im_size, self.y1*self.im_size,
                                                              self.y2*self.im_size, self.psf_sigma)

    def addNoise(self):
        
        if self.debug_bit>2: print("addNoise()")
    ...
    def find(self):
        ...
        if self.debug_bit>1: print("find()")
