...
class Streak:
    ...
    def __init__(self, snr=None, transpose=False, threshold=None, peak=None, foldings=None, subframe=None, section=None):
        ...
        # 3D index of the maximum of the subframe (streak position). from function argument
        self.radon_max_idx = peak
        ...
        # a map of the peak region, with only the part above the cut (peak-num_snr_peak_region) not zeroed out (used for error estimates)
...
#        f = math.fabs(math.sin(math.radians(self.th)))
        ...
        S = round(self.psf_sigma*self.num_psfs_peak_region)
        x1 = xmax - S
        if x1 < 0:
            x1 = 0
        x2 = xmax + S
    ...
    def plotLines(self, ax=None, offset=10, im_type='section', line_format='--m', linewidth=2):
        ...
        if im_type == 'section':
            ...
            raise RuntimeError(
                'Unknown im_type: %s. Use "section" or "full" or "cutout".' % ('section'))
